  ...
  const onSubmit = async (data: SignUpForm) => {
    ...
    try {
      ...
      const { data: authData, error: authError } = await supabase.auth.signUp({
        ...
        options: {
          data: { role: "business" },
          emailRedirectTo: `${window.location.origin}/auth/callback?next=/onboarding/business`,
        },
      });
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      const { error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo: `${window.location.origin}/auth/callback?next=/onboarding/business`,
          queryParams: {
            access_type: "offline",
            prompt: "consent",
          },
    ...
    return (
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-8 text-center">
          ...
          <Link href="/business/login" className="text-emerald-600 hover:text-emerald-700 font-medium">
    ...
    <div className="w-full max-w-md">
      <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-600 mt-2">
            Create your business account to post challenges
          </p>
        </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full pl-12 pr-12 py-3 rounded-xl border bg-white text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all ${errors.confirmPassword ? "border-red-500" : "border-slate-200"}`}
              />
      ...
      <div className="text-center mt-6 space-y-2">
        <p className="text-slate-600">
          ...
          <Link href="/business/login" className="text-emerald-600 hover:text-emerald-700 font-medium">Log in</Link>
        </p>
        <p className="text-slate-500 text-sm">
          Are you a student?{" "}
          <Link href="/student/signup" className="text-primary hover:text-primary-dark font-medium">Sign up here</Link>
        </p>
      </div>
    </div>
  );
